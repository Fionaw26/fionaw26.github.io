...
function highScores() {
    ...
    for (let i = 0; i < min(sortScores.length, 10); i++) {
        ...
        textAlign(RIGHT)
        text(sortScores[i].score, VIRTUAL_WIDTH / 4 - 20, 60 + (i + 1) * 13)
    }
    textAlign(CENTER)
    textSize(8)
...
function checkIfHighScore() {
    ...
    // add if fewer than 10 high scores
...
function victoryState() {
    ...
    textSize(14)
    text("Press Enter to serve", VIRTUAL_WIDTH / 2, 3 * VIRTUAL_HEIGHT / 4)
}
